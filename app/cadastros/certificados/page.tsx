...
import { Shield, Plus, Upload, Download, AlertTriangle, CheckCircle, XCircle } from "lucide-react"
    ...
    <div className="flex-1 space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Gerencie os certificados digitais da empresa</p>
        </div>
        <Dialog>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>Faça upload de um novo certificado digital</DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-gray-500">Formatos aceitos: .pfx, .p12 (Certificado A1)</p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="descricao" placeholder="Ex: Certificado para NFe - Matriz" />
                </div>
              </div>

              {certificadoFile && (
                <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <h4 className="font-semibold text-blue-900 mb-2">Prévia do Certificado</h4>
                  <div className="space-y-1 text-sm text-blue-700">
                    <p>
                      <strong>Arquivo:</strong> {certificadoFile.name}
                    </p>
                    <p>
                      <strong>Tamanho:</strong> {(certificadoFile.size / 1024).toFixed(2)} KB
                    </p>
                    <p>
                      <strong>Tipo:</strong> {certificadoFile.type || "application/x-pkcs12"}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button disabled={!certificadoFile}>Adicionar Certificado</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-lg">{certificado.nome}</CardTitle>
                    <CardDescription>CNPJ: {certificado.cnpj}</CardDescription>
                  </div>
                </div>
                <Badge variant={getStatusColor(certificado.status)}>{certificado.status}</Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                  ...
                  <p className="font-medium">{new Date(certificado.validoAte).toLocaleDateString("pt-BR")}</p>
                </div>
                <div>
                  <span className="text-gray-500">Dias restantes:</span>
                  <p className={`font-medium ${certificado.diasRestantes <= 30 ? "text-red-600" : "text-green-600"}`}>
